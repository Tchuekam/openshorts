  ...
  return (
    ...
    <div className="flex h-screen supports-[height:100dvh]:h-[100dvh] bg-paper overflow-hidden">
            ...
            {keysMissing && (
              ...
              >
                ...
                <span className="hidden md:inline">
                  {!geminiOk ? 'Gemini API Key Missing' : 'Upload-Post API Key Missing'}
                </span>
                <span className="md:hidden">{!geminiOk ? 'key missing' : 'social key missing'}</span>
              </button>
            )}
          </div>
        </header>
        ...
        {keysMissing && activeTab !== 'settings' && (
          <div className="mx-3 sm:mx-6 mt-3 px-3.5 sm:px-4 py-3 bg-paper2 border border-rule rounded-card flex flex-wrap items-center justify-between gap-2.5 sm:gap-4 shrink-0 animate-fade">
            <div className="flex items-start sm:items-center gap-2.5 sm:gap-3 text-sm text-ink2 min-w-0 flex-1">
              ...
              <div className="min-w-0">
                <span className="font-medium text-ink">{!geminiOk ? 'Required API key missing.' : 'Optional social key missing.'}</span>{' '}
                <span className="text-muted">
                  {!geminiOk
                    ? 'Set your Gemini API key to use OpenShorts.'
                    : 'Connect your Upload-Post API key if you want one-click publishing.'}
                </span>
              </div>
            </div>
            <button
            ...
            <div className="h-full overflow-y-auto p-4 sm:p-8 max-w-2xl mx-auto animate-fade">
              ...
              <div className="card p-4 sm:p-6 mt-8">
                ...
                <p className="text-xs text-muted mb-6 leading-relaxed">
                  Optional for direct publishing to TikTok, Instagram Reels, and YouTube Shorts via <strong>Upload-Post</strong>.
                  Connect it only if you want one-click posting; it includes a <strong>free tier</strong> (no credit card required).
                </p>
      ...
      <Modal
        ...
        eyebrow="SETUP"
        title={!geminiOk ? 'Gemini API Key Required' : 'Upload-Post API Key Optional'}
        footer={
      ...
      >
        <div className="space-y-4">
          <p className="text-sm text-muted">
            OpenShorts needs a <strong className="text-ink2">Gemini</strong> API key for the core workflow. <strong className="text-ink2">Upload-Post</strong> is optional and only needed for direct publishing.
          </p>
