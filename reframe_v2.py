...
def _analyze_trajectory(input_video, scenes_boundaries, scene_strategies,
    ...
    total_expected_frames = scenes_boundaries[-1][1] if scenes_boundaries else 0
    xs = []
    frame_number = 0
    ...
    try:
        while True:
            ...
            frame_number += 1
            if total_expected_frames > 0 and (frame_number % max(60, total_expected_frames // 10) == 0 or frame_number == total_expected_frames):
                pct = min(100, int(frame_number * 100 / total_expected_frames))
                print(f"   🎥 Tracking camera trajectory: {frame_number}/{total_expected_frames} frames ({pct}%)...", flush=True)
    finally:
        proc.stdout.close()
        proc.wait()
...
def render(input_video, final_output_video, aspect_ratio, content_ranges=None,
    ...
    try:
        total_scenes = len(ranges)
        completed_scenes = 0
        render_lock = threading.Lock()

        def _render_one_scene(item):
            ...
            with render_lock:
                completed_scenes += 1
                pct = int(completed_scenes * 100 / total_scenes)
                print(f"   🎬 Rendered scene {completed_scenes}/{total_scenes} ({pct}%) [{strategy} {dur:.1f}s]", flush=True)

            return idx, seg_path

        max_workers = min(2, total_scenes)
        segments_by_idx = {}
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            for idx, seg_p in executor.map(_render_one_scene, enumerate(ranges)):
                segments_by_idx[idx] = seg_p
        segments = [segments_by_idx[i] for i in range(total_scenes)]
