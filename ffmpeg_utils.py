...
# Ensure stdout/stderr handle UTF-8 safely on Windows
...
# Uses Intel/AMD/NVIDIA GPU hardware encoding on Windows via MediaFoundation MFT.
_MF_ARGS = {
    QUALITY: ["-c:v", "h264_mf", "-rate_control", "quality", "-quality", "85", "-pix_fmt", "yuv420p"],
    QUALITY_FAST: ["-c:v", "h264_mf", "-rate_control", "quality", "-quality", "75", "-pix_fmt", "yuv420p"],
    DELIVERY: ["-c:v", "h264_mf", "-rate_control", "quality", "-quality", "70", "-pix_fmt", "yuv420p"],
}
...
_mf_ok = None     # None = not probed yet
_announced = False
...
def _probe_mf():
    """One tiny lavfi encode to prove h264_mf (Windows MediaFoundation) works."""
    cmd = [
        "ffmpeg", "-hide_banner", "-loglevel", "error",
        "-f", "lavfi", "-i", "color=black:s=256x256:d=0.1",
        "-c:v", "h264_mf", "-f", "null", "-",
...
def mf_available():
    """Probe h264_mf once and cache the verdict (thread-safe)."""
    global _mf_ok
    if _mf_ok is None:
        with _probe_lock:
            if _mf_ok is None:
...
def reset_encoder_cache():
    ...
    global _nvenc_ok, _mf_ok, _announced
    with _probe_lock:
        _nvenc_ok = None
        _mf_ok = None
        _announced = False
...
def video_encode_args(tier=QUALITY):
    ...
    if mode in ("nvenc", "auto") and nvenc_available():
        ...
        selected_args = _NVENC_ARGS
    elif mode in ("mf", "auto") and mf_available():
        ...
        if mf_available():
            ...
            print("⚠️ [Encoder] FFMPEG_ENCODER=mf requested but not available — falling back to libx264")

    if not _announced:
        _announced = True
        print(f"🎞️ [Encoder] video encoder: {selected_encoder} (FFMPEG_ENCODER={mode})")

    return list(selected_args[tier])
