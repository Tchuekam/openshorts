...
def get_visual_clips(video_path, video_duration, language="en"):
    ...
    try:
        ...
        # Clamp to the real duration; split or drop degenerate/oversized clips.
        clean = []
        for s in shorts:
            st = max(0.0, float(s.get("start", 0)))
            en = min(float(video_duration), float(s.get("end", 0)))
            dur = en - st
            if dur < 1.0:
                continue

            # If the vision model returned an overly long clip (e.g. whole video > v_max_secs),
            # split it into optimal 30-50s Short segments instead of rendering a massive video.
            if dur > v_max_secs and float(video_duration) > v_max_secs:
                print(f"⚠️ Vision clip {st:.1f}s–{en:.1f}s ({dur:.1f}s) exceeds max short length ({v_max_secs}s). Intelligently segmenting.")
                cur = st
                part = 1
                target_len = min(45.0, v_max_secs)
                while cur + v_min_secs <= en:
                    next_end = min(cur + target_len, en)
                    if (en - next_end) < v_min_secs and (en - cur) <= (v_max_secs + 5.0):
                        next_end = en  # absorb small tail
                    part_clip = dict(s)
                    part_clip["start"] = round(cur, 3)
                    part_clip["end"] = round(next_end, 3)
                    if part > 1 or (cur + target_len < en):
                        part_clip["viral_hook_text"] = f"{s.get('viral_hook_text', '')} (Part {part})".strip()
                        part_clip["video_title_for_youtube_short"] = f"{s.get('video_title_for_youtube_short', '')} - Part {part}".strip()
                    clean.append(part_clip)
                    cur = next_end
                    part += 1
                    if cur >= en:
                        break
